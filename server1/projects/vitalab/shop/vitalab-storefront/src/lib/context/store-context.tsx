...
interface StoreContext {
  ...
  countAnalysis: (arr: Array<LineItem>) => number
}
...
export const StoreProvider = ({ children }: StoreProps) => {
  ...
  const countAnalysis = (arr) => {
    const filteredArray = arr.filter(
      (item) => item.variant.product.metadata.category.name !== "Маніпуляційна"
    )
    return filteredArray.reduce((count) => count + 1, 0)
  }
