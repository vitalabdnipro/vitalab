...
const ResetPassword = () => {
  ...
  const onSubmit = handleSubmit(async (credentials: { email: string }) => {
    ...
    const data = await medusaClient.customers.generatePasswordToken({
      email: credentials.email,
    })

    data.response.status === 204 && setPasswordReset(true)
  })
  ...
  return (
    <div className="flex w-full max-w-sm flex-col items-center">
      {!passwordReset ? (
        <>
          <h1 className="text-large-semi mb-4">
            Відновлення пароля від особистого кабінету
          </h1>
          <p className="mb-10 text-center text-sm text-gray-700">
        ...
        <>
          <div className="">
            <h3 className="text-xl font-semibold">
              Ми надіслали вам електронний лист з подальшими інструкціями щодо
              зміни пароля.
            </h3>
            <p className="mt-4 text-sm">
              Якщо ви не отримали електронний лист протягом 10 хвилин, перевірте
              спам або повторіть спробу.
            </p>
          </div>
        </>
      )}
